    ...
    function transaction($txn = null) {
        switch ($txn) {
            ...
            case 'export':
                ...
                $excel->getActiveSheet()->setCellValue('A8','Product: '.$product_name)
                                        ->getStyle('A8')->getFont()->setBold(True);
                $excel->getActiveSheet()->setCellValue('A9','Date Range: '.date( "F d, Y", strtotime($start_date) ).' - '.date( "F d, Y", strtotime($end_date) ))
                                        ->mergeCells('A9:B9')
                                        ->getStyle('A9')->getFont()->setBold(True);

                $excel->getActiveSheet()->getStyle('A10:G10')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                $excel->getActiveSheet()->getStyle('A10:G10')->getFont()->setBold(True);
                $excel->getActiveSheet()->setCellValue('A10','PLU')
                                        ->setCellValue('B10','Product Description')
                                        ->setCellValue('C10','Unit')
                                        ->setCellValue('D10','Price')
                                        ->setCellValue('E10','Supplier')
                                        ->setCellValue('F10','Date Invoice')
                                        ->setCellValue('G10','Reference No');

                $i=11;
                foreach ($data as $data) {
                   $excel->getActiveSheet()->getStyle('F'.$i.':'.'G'.$i)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
                   $excel->getActiveSheet()->getStyle('D'.$i)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
   
                   $excel->getActiveSheet()->setCellValue('A'.$i,$data->product_code);
                   $excel->getActiveSheet()->setCellValue('B'.$i,$data->product_desc);
                   $excel->getActiveSheet()->setCellValue('C'.$i,$data->unit_name);
                   $excel->getActiveSheet()->getStyle('D'.$i)->getNumberFormat()->setFormatCode('###,##0.00;(###,##0.00)');    
                   $excel->getActiveSheet()->setCellValue('D'.$i,$data->dr_price);
                   $excel->getActiveSheet()->setCellValue('E'.$i,$data->supplier_name);
                   $excel->getActiveSheet()->setCellValue('F'.$i,$data->date_delivered);
                   $excel->getActiveSheet()->setCellValue('G'.$i,$data->dr_invoice_no);
                   $i++;
                }
            ...
            case 'email':
                ...
                $data = ob_get_clean();
                          ...
                          $file_name='Purchase Monitoring'.date('Y-m-d h:i:A', now());
